...
public class DbSubjectRepository : ISubjectRepository
{
    ...
    public async Task<Subject?> Add(string name, string url)
    {
        ...
        await _dbContext.Accounts.AddAsync(subject);
        try
        {
    ...
    {
        return await _dbContext.Accounts.FirstOrDefaultAsync(x => x.Guid == guid);
    }
    ...
    {
        var subject = _dbContext.Accounts.FirstOrDefaultAsync(x => x.Guid == guid);
        ...
        _dbContext.Accounts.Remove(subject);
